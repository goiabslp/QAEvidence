    ...
    const handleSaveGoogleConfig = async () => {
        ...
        try {
            ...
            setSyncStatus(prev => {
                ...
                return { ...prev, progress: 95, message: `Limpando registros excluídos... (${totalProcessed} registros sincronizados)${autoGenMsg}`, finished: false };
            });

            try {
                const { data: cleanData, error: cleanErr } = await supabase.functions.invoke(
                    `google-sheets-sync?action=cleanup_deleted&sheetName=${encodeURIComponent(targetSheetName)}`, 
                    { method: 'POST' }
                );
                ...
                setSyncStatus(prev => {
                    ...
                    if (cleanData?.deletedCount > 0) {
                        cleanMsg = ` | ${cleanData.deletedCount} excluídos removidos.`;
                    }
                    return { ...prev, progress: 100, message: `Sincronização 100% idêntica concluída!${cleanMsg}`, finished: true };
                });
            } catch(e) {
                console.error("Cleanup error", e);
                setSyncStatus(prev => {
                    if (prev.error) return prev;
                    return { ...prev, progress: 100, message: `Sincronização concluída (Pendente limpeza).`, finished: true };
                });
            }
                                            ...
                                            <div className="flex items-start gap-3 w-full">
                                                ...
                                                <div className="flex-1 w-full">
                                                    ...
                                                    <div className="flex flex-col gap-2 mt-2">
                                                        <label className="text-[11px] font-black tracking-wider text-emerald-800 uppercase ml-1">Link da Planilha Mestra</label>
                                                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                                                            <div className="relative flex-1">
                                                                <Link2 className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-emerald-500" />
                                                                <input 
                                                                    type="text" 
                                                                    placeholder="Cole o link ou ID da planilha aqui..." 
                                                                    value={googleUrlInput}
                                                                    onChange={(e) => setGoogleUrlInput(e.target.value)}
                                                                    className="w-full pl-11 pr-4 py-3.5 text-sm font-black text-emerald-900 bg-white border-2 border-emerald-200 rounded-xl focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/20 outline-none transition-all shadow-sm placeholder:text-emerald-300 placeholder:font-medium"
                                                                />
                                                            </div>
                                                            <button
                                                                onClick={handleSaveGoogleConfig}
                                                                disabled={isSavingGoogle || !googleUrlInput}
                                                                className="flex items-center justify-center gap-2 px-6 py-3.5 text-sm font-black text-white bg-emerald-600 rounded-xl hover:bg-emerald-500 transition-all shadow-lg shadow-emerald-600/30 disabled:opacity-50 whitespace-nowrap active:scale-95"
                                                            >
                                                    ...
                                                    {/* Webhook Script Section - Compacted */}
                                                    <div className="mt-5 w-full border-t-2 border-emerald-100/50 pt-5">
                                                        <div className="bg-emerald-50/50 p-4 rounded-xl border border-emerald-100 flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4 transition-all">
                                                            <div className="flex-1">
                                                                <div className="flex items-center gap-2 mb-1">
                                                                    <div className="w-6 h-6 rounded-md bg-emerald-100 flex items-center justify-center shrink-0">
                                                                        <Code2 className="w-3.5 h-3.5 text-emerald-600" />
                                                                    </div>
                                                                    <h5 className="text-sm font-black text-slate-800">Script de Automação (Real-Time)</h5>
                                                                </div>
                                                                <p className="text-[11px] text-slate-500 font-medium">Fundamental para que as atualizações cheguem automaticamente da planilha para o sistema.</p>
                                                            </div>
                                                            
                                                            <div className="flex items-stretch gap-2 w-full xl:w-auto">
                                                                <button 
                                                                    onClick={() => {
                                                                        const code = `function INSTALAR_ROBO_DE_SINCRONIZACAO() {\n  var sheet = SpreadsheetApp.getActive();\n  var triggers = ScriptApp.getProjectTriggers();\n  for (var i = 0; i < triggers.length; i++) {\n    if (triggers[i].getHandlerFunction() === 'MandarParaSistema') {\n      ScriptApp.deleteTrigger(triggers[i]);\n    }\n  }\n  ScriptApp.newTrigger("MandarParaSistema")\n    .forSpreadsheet(sheet)\n    .onEdit()\n    .create();\n    \n  SpreadsheetApp.getUi().alert("✅ DEU CERTO!", "O Robô de Sincronização em Massa foi ativado!", SpreadsheetApp.getUi().ButtonSet.OK);\n}\n\nfunction MandarParaSistema(e) {\n  if (!e || !e.range) return;\n  var sheet = e.source.getActiveSheet();\n  if (sheet.getName().toUpperCase() !== 'TESTES') return;\n  \n  var firstRow = e.range.getRow();\n  var numRows = e.range.getNumRows();\n  \n  if (firstRow <= 1) {\n    numRows = numRows - (2 - firstRow);\n    firstRow = 2; \n  }\n  if (numRows <= 0) return;\n\n  var valuesMatrix = sheet.getRange(firstRow, 1, numRows, 22).getValues();\n  var updates = [];\n\n  for (var i = 0; i < numRows; i++) {\n    var rowValues = valuesMatrix[i];\n    var tagId = rowValues[11];\n    var absoluteRow = firstRow + i;\n    \n    if (tagId) {\n      updates.push({\n        row: absoluteRow,\n        tagId: tagId,\n        values: rowValues\n      });\n    }\n  }\n  \n  if (updates.length === 0) return;\n\n  var payload = {\n    sheetName: sheet.getName(),\n    updates: updates\n  };\n  \n  var url = "https://aqlwziggdwmbrlazpuhv.supabase.co/functions/v1/google-sheets-sync?action=webhook_bulk";\n\n  var options = {\n    method: "post",\n    contentType: "application/json; charset=utf-8",\n    payload: JSON.stringify(payload)\n  };\n\n  try { UrlFetchApp.fetch(url, options); } catch (err) {}\n}`;
                                                                        navigator.clipboard.writeText(code);
                                                                        alert('✅ SCRIPT COPIADO COM SUCESSO!\n\nPasso a passo:\n1. Acesse "Extensões" > "Apps Script" na sua planilha.\n2. Limpe todo o código existente.\n3. Cole o novo script.\n4. Clique em EXECUTAR (▶️).\n5. Aprove as permissões de segurança solicitadas pelo Google.');
                                                                    }}
                                                                    className="flex-1 xl:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-slate-900 text-white font-black rounded-xl hover:bg-slate-800 hover:-translate-y-0.5 focus:ring-4 focus:ring-slate-900/20 active:translate-y-0 active:scale-95 transition-all text-xs shadow-lg shadow-slate-900/10"
                                                                >
                                                                    <div className="bg-white/20 p-1 rounded-md text-emerald-400"><Copy className="w-3.5 h-3.5" /></div>
                                                                    COPIAR SCRIPT E INSTALAR
                                                                </button>
                                                                
                                                                <details className="group relative cursor-pointer">
                                                                    <summary className="list-none flex items-center justify-center w-11 h-11 rounded-xl bg-white border border-emerald-200 text-emerald-600 hover:bg-emerald-50 hover:border-emerald-300 transition-all shadow-sm">
                                                                        <ChevronDown className="w-5 h-5 transition-transform duration-300 group-open:rotate-180" />
                                                                    </summary>
                                                                    <div className="absolute right-0 top-[120%] w-[320px] sm:w-[500px] z-[50] mt-1 origin-top-right animate-in fade-in zoom-in-95 duration-200">
                                                                        <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col">
                                                                            <div className="p-4 bg-yellow-50 border-b border-yellow-100 flex flex-col gap-2">
                                                                                <p className="text-[11px] font-black text-yellow-800 flex items-center gap-1.5"><AlertCircle className="w-3.5 h-3.5" /> Passo a Passo de Configuração:</p>
                                                                                <ol className="text-[10px] text-yellow-800 space-y-1.5 list-decimal ml-4 font-bold">
                                                                                    <li>Acesse <span className="underline">Extensões &gt; Apps Script</span> no menu do Google Sheets.</li>
                                                                                    <li>Cole o conteúdo que você copiou acima, apagando tudo que houver antes.</li>
                                                                                    <li>Clique em <span className="bg-yellow-200/50 px-1 rounded">▶️ Executar</span> no topo da área.</li>
                                                                                    <li>Se surgir um aviso do Google: <strong>Revisar Permissões</strong> &gt; Avançado &gt; Permitir.</li>
                                                                                </ol>
                                                                            </div>
                                                                            <div className="p-4 bg-slate-900">
                                                                               <p className="text-[10px] text-emerald-400/80 mb-2 font-bold uppercase tracking-widest">Prévia do Script:</p>
                                                                               <div className="h-48 overflow-y-auto custom-scrollbar rounded-lg border border-slate-800 p-2">
                                                                                    <pre className="text-[8px] sm:text-[9px] font-mono text-slate-300 leading-relaxed selection:bg-emerald-500/30">
