...
const ScenarioItem = React.memo(({ 
    ...
    return (
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md page-break-avoid will-change-transform">
            <div
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-2">
                            ...
                            {group.items.map((item: any) => (
                                <span
                                    key={item.id}
                                    className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-mono font-bold border shadow-sm ${item.status === TestStatus.FAIL
                    ...
                    <div className="flex items-center justify-end gap-3 pl-14 sm:pl-0 relative z-20">
                        {onAddCase && (
                            <button
                                ...
                                className="flex items-center gap-1.5 text-xs bg-white hover:bg-indigo-50 text-indigo-700 px-3 py-2 rounded-lg border border-slate-200 hover:border-indigo-200 transition-all shadow-sm font-bold uppercase tracking-wide mr-2 relative z-50 capitalize"
                                type="button"
                            >
            ...
            {isScenarioOpen && (
                <div className={`bg-white divide-y divide-slate-100 ${readOnly ? '' : 'animate-slide-down'} scroll-smooth`}>
                    {group.items.map((evidence: any) => (
                        <CaseItem 
                            key={evidence.id} 
                            evidence={evidence} 
                            isCaseOpen={readOnly || expandedCases.has(evidence.id)}
    ...
    <div className={`flex flex-col ${readOnly ? 'gap-6' : 'gap-8'}`}>
      {groupedEvidences.map((group) => {
        ...
        if (group.type === 'scenario') {
          return (
            <ScenarioItem 
                key={`scenario-${group.scenarioNumber}`}
                group={group}
                isScenarioOpen={readOnly || expandedScenarios.has(group.scenarioNumber)}
        ...
        return (
          <div key={evidence.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col md:flex-row overflow-hidden group break-inside-avoid will-change-transform">
